...
public class DistributedLockByCurator implements InitializingBean{
     ...
     * CuratorFramework实例都是线程安全的，我们应该当在ZooKeeper Cluster中共享同一个CuratorFramework实例。
     * 工厂方法newClient() 提供了简便创建client实例方式，使用start和close方法启动和关闭客户端。使用Curator框架创建客户端代码如下，
     */
    @Autowired
    ...
    //创建父节点，并创建永久节点
    //spring容器初始化的时候afterPropertiesSet就会被调用。
    @Override
    public void afterPropertiesSet() {
        curatorFramework = curatorFramework.usingNamespace("lock-namespace");
        String path = "/" + ROOT_PATH_LOCK;
        try {
            if (curatorFramework.checkExists().forPath(path) == null) {// 创建节点
